...
def case_compute_prob_metrics_crps_ensemble_success():
    ...
    # compute_kwargs
    compute_kwargs = {
        'metric_kwargs' : {'crps_ensemble' : {'member_dim' : "member", 'keep_attrs' : True}},
        'return_on_error' : None
    }

    # creating expected df. expected_xr is added later due to pd dtype issues
    expected_df = pd.DataFrame({"band": ['member'], "crps_ensemble": [None]})
...
def case_compute_prob_metrics_crps_gaussian_success():
    ...
    # creating expected df. expected_xr is added later due to pd dtype issues
    expected_df = pd.DataFrame({"band": ['1'], "crps_gaussian": [None]})
...
def case_compute_prob_metrics_crps_quadrature_success():
    ...
    size = 5
    expected_xr = xr.Dataset(data_vars={'variable' : 174.86983819}, coords={'spatial_ref': 0})
    band_dim_name = 'band'

    # compute_kwargs
    compute_kwargs = {
        'metric_kwargs' : {'crps_quadrature' : {
            'cdf_or_dist' : norm, 'tol' : 1e-6, 'keep_attrs' : False
            }
        },
        'return_on_error' : None
    }
    ...
    # creating expected df. expected_xr is added later due to pd dtype issues
    expected_df = pd.DataFrame({"band": ['1'], "crps_quadrature": [None]})
...
def case_compute_prob_metrics_discrimination_success():
    ...
    # creating expected df. expected_xr is added later due to pd dtype issues
    expected_df = pd.DataFrame({"band": ['1'], "discrimination": [None]})
...
def case_compute_prob_metrics_rank_histogram_success():
    ...
    # creating expected df. expected_xr is added later due to pd dtype issues
    expected_df = pd.DataFrame({"band": ['member'], "rank_histogram": [None]})
...
def case_compute_prob_metrics_reliability_success():
    ...
    # compute_kwargs
    compute_kwargs = {
        'metric_kwargs' : {'reliability' : {'probability_bin_edges' : np.array([0., .1, .3, .5, .7, .8, .9])}},
        'return_on_error' : None
    }

    # creating expected df. expected_xr is added later due to pd dtype issues
    expected_df = pd.DataFrame({"band": ['1'], "reliability": [None]})
...
def case_compute_prob_metrics_roc_success():
    ...
    # creating expected df. expected_xr is added later due to pd dtype issues
    expected_df = pd.DataFrame({"band": ['1'], "roc": [None]})
...
def case_compute_prob_metrics_rps_success():
    ...
    # creating expected df. expected_xr is added later due to pd dtype issues
    expected_df = pd.DataFrame({"band": ['member'], "rps": [None]})
    expected_df.loc[0, 'rps'] = expected_xr

    # Band parameters for candidate
    # background value, circle value, circle center, and circle radius
    band_params_candidate = [
        (0.08, .92, (9, 9), 3),  # Band 1
        (.76, 0.1, (14, 14), 5),  # Band 2
...
def case_compute_prob_metrics_threshold_brier_score_success():
    ...
    # compute_kwargs
    compute_kwargs = {
        'metric_kwargs' : {'threshold_brier_score' : {
    ...
    # creating expected df. expected_xr is added later due to pd dtype issues
    expected_df = pd.DataFrame({"band": ['member'], "threshold_brier_score": [None]})
...
def case_compute_prob_metrics_error_success():
    ...
    # generate xarray pairs
    candidate_map, benchmark_map = _create_xarray_pairs(
        upper_left, lower_right, sizes, band_params_candidate, band_params_benchmark, None, encoded_nodata_value, shapes, band_dim_name, return_datasets
    )
...
def case_compute_prob_metrics_kwargs_required_fail():
    ...
    # compute_kwargs
    compute_kwargs = {
        'metric_kwargs' : {'crps_gaussian' : None},
        'return_on_error' : None
    }
