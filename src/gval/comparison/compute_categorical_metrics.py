...
# @pa.check_types
def _compute_multiclass_categorical_metrics_aggregate_form(
    crosstab_df: DataFrame[Crosstab_df],
    metrics: Union[str, Iterable[str]] = "all",
    aggregation_method: str = "macro",
) -> DataFrame[Metrics_df]:
    """
    Computes multiple categorical metrics from a crosstab df.
    """

    # make copy to avoid modifying original
    crosstab_df = crosstab_df.copy()

    # for each categorical value, compute the metrics
    def compute_metrics_per_sample(sample):
        """Computes the metric values on a per sample basis"""

        # pivots crosstab df to NxM matrix where N is the number of candidate values and M is the number of benchmark values. The values in each cell are the counts of the pairing.
        crosstab_df_pivoted = sample.pivot(
            index="candidate_values", columns="benchmark_values", values="counts"
        )

        # FIXME: Temporary functionality until CatStats.process_statistics or a version of it is implemented for multiclass cases
        metric_dict = {
            mfunc(crosstab_df_pivoted, aggregation_method)
            for mname, mfunc in metrics.items()
        }

        # compute metric values and return names
        # metric_values, metric_names = CatStats.process_statistics(
        #    metrics, **crosstab_df_pivoted.to_dict()
        # )
